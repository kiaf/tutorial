...
cp /pool/macro/job_wrap.sh /pool/macro/multi_merge.sh ${work_dir}/macro/.
tr_out_files=`echo $out_files | sed -e 's/,/ /g'`
...
m$l.OutputFile("${main_dir}/merge${l}_${out_file_array[$l]}");
EOF
done
...
for (( l=0 ; l<nout_files ; l++ )) ; do
cat <<EOF >> ${work_dir}/macro/merge$l.C
m$l.AddFile("${main_dir}/${work_dir}/${num}/${out_file_array[$l]}");
EOF
done
k=$((k + 1))
fi
...
for (( l=0 ; l<nout_files ; l++ )) ; do
echo "Job MERGE$l ${work_dir}/macro/merge$l.jdl" >> ${work_dir}/macro/condor.dag
done
echo "PARENT \ " >> ${work_dir}/macro/condor.dag
cat ${work_dir}/macro/condor.parents.list >> ${work_dir}/macro/condor.dag
echo "CHILD \ " >> ${work_dir}/macro/condor.dag
for (( l=0 ; l<nout_files ; l++ )) ; do
echo "MERGE$l \ " >> ${work_dir}/macro/condor.dag
cat <<EOF >> ${work_dir}/macro/merge$l.C
m$l.Merge();
}
EOF
